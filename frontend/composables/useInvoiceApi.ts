...
export function useInvoiceApi() {
  ...
  // Create invoice
  const createInvoice = async (invoiceData: Omit<Invoice, 'id' | 'created_at' | 'updated_at'>): Promise<Invoice> => {
    try {
      isLoading.value = true;
      clearMessages();

      const response = await $fetch<Invoice>(`${apiBase}/invoices`, {
        method: 'POST',
        body: invoiceData,
      });
      ...
      error.value = 'Failed to create invoice. Please check the form and try again.';
      console.error('API Error:', err);
      throw err;
    } finally {
      isLoading.value = false;
